    ...
    extends AbstractCommonModuleObjectAttributeValueTypesComputer
{
    ...
    public List<TypeItem> getTypes(Invocation inv)
    {
        ...
        List<TypeItem> types = this.getTypesComputer().computeTypes(dataExpression, envs.environments());
        if (types.isEmpty())
            return Collections.emptyList();
            ...
            && ((Type)type).getCollectionElementTypes().allTypes().get(0) instanceof Type)
        {
            ...
            {
                String expressionContent = getExpressionContent(nameExpression);
                if (property.getName().equals(expressionContent))
                {
